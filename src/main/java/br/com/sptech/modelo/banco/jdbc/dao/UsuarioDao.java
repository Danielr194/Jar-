...
public class UsuarioDao {
    ...
    public Integer salvarUsuario(ModelUsuario novoUsuario) {
        ...
        con.update("INSERT INTO usuario (nome, email, senha, palavraChave, area, cargo, fk_empresa, fk_tipo_usuario) VALUES (?, ?, ?, ?,?, ?, ?, ?)", novoUsuario.getNome(), novoUsuario.getEmail(), novoUsuario.getSenha(), novoUsuario.getFraseSeguranca(), novoUsuario.getArea(), novoUsuario.getCargo(), novoUsuario.getFkEmpresa(), novoUsuario.getFkTipoUsuario());
    ...
    public Boolean isUsuarioExistente(ModelUsuario usuario) {
        ...
        String sql = "SELECT COUNT(*) FROM usuario WHERE email = ? AND senha = ? and palavraChave = ?";
        Integer count = con.queryForObject(sql, Integer.class, usuario.getEmail(), usuario.getSenha(), usuario.getFraseSeguranca());
